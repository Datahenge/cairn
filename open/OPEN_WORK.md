...
| `W-012` | `in_progress` | `DOCS` | Rewrite `README.md` and `docs/technical/CONFIGURATION.md` for the `cairn-build`/`cairn-adopt` split (`ADR-046`) — both still describe the retired unified `cairn` binary and `cairn-provision` | `userdocs/get-started/index.md` now covers prerequisites, install, and `doctor`, verified live against a client test VPS (2026-08-03) — no longer forwards those topics to the stale README. Manifest-writing and first build are the next sections to verify and write, then `README.md` gets cut down to a pointer at the published site |
