...
| `W-021` | `done` | `CLI` | `build --push` now assigns the manifest's declared environment by default; `--no-assign-tag` opts out; a manifest with no environment is silently skipped, not errored. The `:production` gate needed no new wiring — it was already keyed off the target environment, not how assignment was requested | Decided by Brian 2026-08-05 (`ADR-066`, `decisions/066-build-push-defaults-to-assign-tag.md`); `src/cairn/cli_build.py`, `docs/requirements/06-cli.md` (`BR-CLI-002a`), `tests/test_cli_build.py` |
