...
| `W-012` | `in_progress` | `DOCS` | Rewrite `README.md` and `docs/technical/CONFIGURATION.md` for the `cairn-build`/`cairn-adopt` split (`ADR-046`) — both still describe the retired unified `cairn` binary and `cairn-provision`. Now also stale on `/etc/cairn/environment.toml` → `/etc/cairn/adopt.toml` (`ADR-034` amendment, 2026-08-03) | `userdocs/get-started/index.md` now covers prerequisites, install, and `doctor`, verified live against a client test VPS (2026-08-03) — no longer forwards those topics to the stale README. Manifest-writing and first build are the next sections to verify and write, then `README.md` gets cut down to a pointer at the published site |
| `W-013` | `open` | `CLI` | Implement `BR-CLI-022`/`BR-CLI-023` (`ADR-047`): `cairn-build setup --client <name>` provisioning `/srv/cairn/<name>/` + scaffolding a starter `cairn.toml` when absent; split the `timers` stage out of both `setup` commands into new `cairn-build setup-timer`/`cairn-adopt setup-timer` | `src/cairn/provision.py` (`BUILD_STAGES`/`ADOPT_STAGE_FUNCS`, `stage_registry`-style new stage), `src/cairn/cli_build.py`/`cli_adopt.py` (new `--client` option, new `setup-timer` command). Requirements landed 2026-08-03; code not yet written. Blocks finishing the Builder guide's manifest section |
