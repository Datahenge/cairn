...
| `W-016` | `done` | `CFG`, `CLI` | The `[cairn.environments]` → `[cairn.declared_environments]` rename (`ADR-049`) landed 2026-08-04: `config.py`, `environments.py`, `provision.py`'s scaffolded manifest, `cli_registry.py`/`registry_retention.py` comments, plus `README.md` and every `userdocs/` reference. Tests updated (`test_environments.py`, `test_config.py`, `test_registry_retention.py`, `test_cli_build.py`), full suite + ruff clean | `decisions/049-the-declared-environment-list-is-cairn-declared-environments-not.md`; unexercised against a real registry, same gap as the rest of the deploy path (`open/OPEN_WORK.md`'s `W-001`) |
| `W-017` | `done` | `CLI` | The `new-tag`/`retag` → `assign-tag` merge (`ADR-050`) landed 2026-08-04: `environments.assert_creating`/`assert_moving` collapsed into `environments.action()` (reports "created"/"moved", never refuses), `new_tag_command`/`retag_command` collapsed into `assign_tag_command` in `cli_build.py`, the `setup-timer` build script now calls `assign-tag`. Drive-by fix in the same pass: `setup-timer`'s printed header always said "setup" regardless of subcommand (`setup_runner.execute`'s new `verb` param) — unrelated to `ADR-050` but found while touching the same call sites. Tests, `README.md`, `userdocs/` all updated | `docs/adr/050-new-tag-and-retag-merge-into-assign-tag.md`; landed together with `W-016`; unexercised against a real registry, same gap as `W-016` |
