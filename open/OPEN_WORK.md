...
| `W-018` | `open` | `BUILD`, `CLI` | Implement `ADR-051`: add `cairn-build prune --keep 1 --yes` as a fourth line in `provision.py`'s generated build script, after `build --push` and `assign-tag` | `decisions/051-cairn-build-prune-runs-inside-the-build-script-not-a-separate-timer.md`; do this together with `W-016`/`W-017` since all three touch the same generated script |
| `W-016` | `open` | `CFG`, `CLI` | Implement the `[cairn.environments]` → `[cairn.declared_environments]` rename (`ADR-049`): `config.py`, `environments.py`, `tests/test_environments.py`, `tests/test_config.py`, `tests/test_registry_retention.py`, plus `README.md` and every `userdocs/` reference (`reference/manifest.md`, `reference/target-descriptor.md`, `registry/index.md`) in the same change | `decisions/049-the-declared-environment-list-is-cairn-declared-environments-not.md`; requirements text already updated (`docs/requirements/02-build.md`, `03-deploy.md`, `00-overview.md`, `08-registry.md`) |
| `W-017` | `open` | `CLI` | Implement the `new-tag`/`retag` → `assign-tag` merge (`ADR-050`): collapse `environments.assert_creating`/`assert_moving` into one reporting path, collapse `new_tag_command`/`retag_command` into `assign_tag_command` in `cli_build.py`, update the `setup-timer` build script (`provision.py`) to call `assign-tag` instead of `retag`, update tests, `README.md`, and `userdocs/` | `docs/adr/050-new-tag-and-retag-merge-into-assign-tag.md`; requirements text already updated (`docs/requirements/06-cli.md`, `03-deploy.md`); do this together with `W-016` since both touch `environments.py`/`cli_build.py` and both need the same userdocs pass |
