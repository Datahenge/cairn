...
| `DOCS-01` | `needs_user` | `DOCS` | `docs/technical/ABOUT_GHCR.md` (~3,200 words) has a clean split into setup / ownership-and-cost / tags-and-troubleshooting sub-topics. `docs/CHANGELOG.md` (2026-08-03, `ADR-045` entry) already records that this file — along with `README.md`, `CONFIGURATION.md`, `ABOUT_REGISTRIES.md` — is slated for eventual migration into the `userdocs/` mkdocs nav as separate future work. Split it now in `docs/technical/` (risks being redone at migration time), defer any restructuring until that migration, or split it now shaped deliberately to become the future mkdocs pages directly? | No recommendation recorded yet | — |
