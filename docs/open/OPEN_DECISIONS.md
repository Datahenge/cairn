...
| `ADR-073` | `needs_user` | `DEPLOY` / `CLI` | Whether `cairn-adopt` gains target-stack lifecycle (`stop`/`start`/`restart`) plus a hold state `reconcile` respects, and/or a write-through `.env` so plain `docker compose` stays correct | Scope settled by Brian 2026-08-18 (an operator has no practical alternative route); the design is not. Claude's lean: the hold state is the primary deliverable, since a `stop` that loses a race with `cairn-reconcile.timer` appears to have worked. Two sub-questions need Brian: whether a hold survives reboot, and whether cairn writing `.env` collides with `BR-DEPLOY-011` (narrowed 2026-08-18 — on the one live host inspected, the compose file interpolates no secret-bearing variable, so no collision arises there; the ruling is still needed for the general case) | [../adr/073-target-stack-lifecycle-and-a-maintenance-state.md](../adr/073-target-stack-lifecycle-and-a-maintenance-state.md) |
