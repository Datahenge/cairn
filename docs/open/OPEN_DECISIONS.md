...
| `ADR-071` | `deferred` | `CLI` | Whether `cairn-build doctor --all` (`ADR-070`) should extend beyond the build-timer check to a full per-manifest audit of `config` and `github reachability` too | Deferred at Brian's request 2026-08-06 — ship the narrower, timer-only `--all` first; revisit once it's proven out, doing the `check_config` split and the `check_known_manifests` silent-parse-failure fix together rather than as a side effect | [../adr/071-doctor-all-flag-extended-to-config-and-github-reachability.md](../adr/071-doctor-all-flag-extended-to-config-and-github-reachability.md) |
| `ADR-073` | `approved` | `DEPLOY` / `CLI` | **Narrowed 2026-08-18 by `ADR-074`**, which settled the `.env` half (it is permanent architecture, not a bridge). **Direction approved 2026-08-18**: candidate (b) — cairn offers its own `up`/`down` verbs, and a human never runs `docker compose` directly (candidate (a) rejected outright, `W-037`). Fully specified the same day: durable hold at `/etc/cairn/hold`, verbs `start`/`stop`. Nothing further needed before implementation (`W-035`) |  Scope settled by Brian 2026-08-18 (an operator has no practical alternative route); the design is not. Claude's lean: the hold state is the primary deliverable, since a `stop` that loses a race with `cairn-reconcile.timer` appears to have worked. Two sub-questions need Brian: whether a hold survives reboot, and whether cairn writing `.env` collides with `BR-DEPLOY-011` (narrowed 2026-08-18 — on the one live host inspected, the compose file interpolates no secret-bearing variable, so no collision arises there; the ruling is still needed for the general case) | [../adr/073-target-stack-lifecycle-and-a-maintenance-state.md](../adr/073-target-stack-lifecycle-and-a-maintenance-state.md) |
