...
| ID | Status | Area | Work | Notes / Links |
| --- | --- | --- | --- | --- |
| `W-021` | `done` | `CLI` | `build --push` now assigns the manifest's declared environment by default; `--no-assign-tag` opts out; a manifest with no environment is silently skipped, not errored. The `:production` gate needed no new wiring — it was already keyed off the target environment, not how assignment was requested | Decided by Brian 2026-08-05 (`ADR-066`, `docs/decisions/066-build-push-defaults-to-assign-tag.md`); `src/cairn/cli_build.py`, `docs/requirements/06-cli.md` (`BR-CLI-002a`), `tests/test_cli_build.py` |
| `W-011` | `done` | `DOCS` | Moved `docs/plans/phase-1-build.md` to `docs/archive/phase-1-build.md` — its frontmatter already said `status: archived`; it was simply misfiled in the live `docs/plans/` tree. Not refreshed into a Phase-4 plan since `docs/requirements/` + ADRs already superseded its content in place | `docs/archive/README.md`'s "archived plans" index |
