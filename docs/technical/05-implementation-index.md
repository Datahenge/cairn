...
| Reconcile / deploy | `BR-DEPLOY-*` | Written, unverified live | Heavily tested and mutation-checked; **never run on a real target**. 2026-08-04: `[cairn.environments]` renamed `[cairn.declared_environments]` (`ADR-049`) and `new-tag`/`retag` merged into `assign-tag` (`ADR-050`), which creates a pointer if absent instead of refusing — closes the `setup-timer` bootstrap gap this same session found | `src/cairn/reconcile.py`, `src/cairn/descriptor.py`, `src/cairn/environments.py`, `src/cairn/config.py`, `src/cairn/cli_build.py` | `tests/test_reconcile.py`, `tests/test_descriptor.py`, `tests/test_environments.py`, `tests/test_config.py`, `tests/test_cli_build.py` | `docs/requirements/03-deploy.md`, `docs/requirements/06-cli.md` | First live deployment — see `open/OPEN_WORK.md`; `assign-tag`/`declared_environments` unexercised against a real registry |
...
| Manifest home + setup-timer split | `BR-CLI-022/023` | Implemented | `cairn-build setup --client <name>` provisions `/srv/cairn/<name>/`, scaffolds a starter `cairn.toml` only if absent; `setup-timer` split off both CLIs' `setup`; `doctor` lists known manifests informationally. 2026-08-04: the generated build script now runs `build --push`, `assign-tag --latest --yes` (`ADR-050`), then `prune --keep 1 --yes` (`ADR-051`) in one pass — local disk cleanup rides the same script rather than a fourth timer; also fixed a `setup-timer` header that always printed "setup" regardless of which subcommand ran | `src/cairn/provision.py` (`stage_manifest`, `MANIFEST_ROOT`, `MANIFEST_TEMPLATE`, `build_script`, `_require_root`), `src/cairn/setup_runner.py` (`execute`'s `verb` param), `src/cairn/doctor.py` (`check_known_manifests`), `src/cairn/cli_build.py`, `src/cairn/cli_adopt.py` | `tests/test_provision.py`, `tests/test_setup_runner.py`, `tests/test_doctor.py`, `tests/test_cli_build.py`, `tests/test_cli_adopt.py` | `docs/requirements/06-cli.md`, `docs/adr/047-canonical-manifest-home-srv-cairn-scaffolding-and-setup-timer.md`, `docs/adr/050-new-tag-and-retag-merge-into-assign-tag.md`, `decisions/051-cairn-build-prune-runs-inside-the-build-script-not-a-separate-timer.md` | `setup --client` and the scaffolded manifest verified live 2026-08-04 (a client VPS: `doctor` 9/9, `build --dry-run`, then a real `build`, 5m15s). `setup-timer`, its `assign-tag`/`prune` script, and `doctor`'s known-manifests listing still unexercised against a real host (`W-013`) |
