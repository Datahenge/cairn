...
MUST be recorded in a committed `.ventwig.lock`. Builds MUST reproduce without network
access to the upstream host. *(ADR-007)*
...
vendored tree matches the content-tree hash in `.ventwig.lock`. On any drift the operation
MUST abort; there is no override. *(ADR-001, ADR-007)*
