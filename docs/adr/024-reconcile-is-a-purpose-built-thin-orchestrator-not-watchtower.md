...
- **Watchtower** — solves only the trivial part (poll a tag's digest, pull, recreate a
  ...
  environments, `CUSTOM_IMAGE`/`CUSTOM_TAG` composition, or health-gated sequencing.
...
once → health → rollback-by-repoint) has no off-the-shelf solution — it is precisely the
connective tissue cairn exists to provide.

*(Originally described this sequence with an optional `install-app` step; `ADR-037` struck
that clause entirely — cairn never installs an app.)*
