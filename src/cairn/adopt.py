...
def descriptor_for(found: Survey, environment: str) -> Descriptor:
    ...
    if found.registry_host is None:
        raise ValueError("the registry host currently deployed could not be determined")
    ...
    return Descriptor(
        ...
        tag=found.tag,
        site=found.site,
        registry_host=found.registry_host,
        compose=Compose(
...
def render(found: Survey, environment: str) -> str:
    ...
    lines = [
        ...
        "",
        f'{"environment":<13} = "{proposed.environment}"',
        # Separate from `image` on purpose — a repository name is one thing, not a registry
        # host and a repository name glued into one string. Read off the running container;
        # a hostless reference means Docker Hub, made explicit as "docker.io" rather than left
        # unstated.
        f'{"registry_host":<13} = "{proposed.registry_host}"',
        f'{"image":<13} = "{proposed.image}"',
        f'{"tag":<13} = "{proposed.tag}"',
        f'{"site":<13} = "{proposed.site}"',
        "",
        "[compose]",
    ]
...
def report(found: Survey, manifest: Manifest | None = None) -> list[str]:
    ...
    if found.image is None:
        repository = "?"
    elif found.registry_host:
        repository = f"{found.registry_host}/{found.image}"
    else:
        repository = found.image
    lines = [
        ...
        f"Running image     {repository}:{found.tag or '?'}",
    ]
...
def _survey_image(found: Survey) -> None:
    ...
    if not base or "/" in tag:
        ...
        return
    found.registry_host, found.image = registry.split_host(base)
    found.tag = tag
